...
const LessonBlock: React.FC<Lessonblockprops > = ({ block }) => {
  return (
    ...
    >
      <Card  />
        <Cardheader >
          <Cardtitle  style={{ fontWeight: "700" }}></Lessonblockprops>
            {block.title}
          </Lessonblockprops>
        </Cardheader>
        <Cardcontent >
          <P className="text-gray-300 leading-relaxed whitespace-pre-line text-lg"></Cardcontent>
            {block.content}
          </Cardcontent>
          ...
          <Div>
            ...
            <Ul className="space-y-3">
              {block.keyTakeaways.map((takeaway, index) => (
                ...
                >
                  <checkcircle2 >
                  <Span className="text-gray-300 text-base"></Ul>{takeaway}</Ul>
                </motion.li>
              ))}
            </Ul>
          </Div>
