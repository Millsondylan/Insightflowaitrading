...
export class MarketSentimentAnalyzer {
    ...
    const newsPromises = Array.from(this.newsAPIs.entries()).map(
      async ([source, apiKey]) => {
        try {
          ...
          return response.data.articles.map((article: any // eslint-disable-line @typescript-eslint/no-explicit-any // eslint-disable-line @typescript-eslint/no-explicit-any) => ({
            title: article.title,
    ...
    const socialPromises = Array.from(this.socialAPIs.entries()).map(
      async ([platform, apiKey]) => {
        try {
          ...
          return response.data.data.map((tweet: any // eslint-disable-line @typescript-eslint/no-explicit-any // eslint-disable-line @typescript-eslint/no-explicit-any) => ({
            content: tweet.text,
            platform: platform,
  ...
  private calculateEngagementWeight(engagement: any // eslint-disable-line @typescript-eslint/no-explicit-any // eslint-disable-line @typescript-eslint/no-explicit-any): number {
