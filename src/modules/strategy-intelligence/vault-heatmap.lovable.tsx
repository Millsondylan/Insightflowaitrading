...
export const VaultHeatmap: React.FC<VaultHeatmapProps> = ({
  ...
  if (loading) {
    return <div style={{ display: "flex", justifyContent: "center", alignItems: "center" }}>Loading heatmap data...</div>;
  }

  if (error) {
    return <div style={{ padding: "16px" }}>Error: {error}</div>;
  }

  if (!heatmapData || !heatmapData.data.length) {
    return <div style={{ padding: "16px" }}>No heatmap data available for this strategy.</div>;
  }
