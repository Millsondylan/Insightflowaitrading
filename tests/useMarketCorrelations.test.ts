...
jest.mock('@/integrations/supabase/client', () => {
  ...
  const subscribers: any // eslint-disable-line @typescript-eslint/no-explicit-any[] = [];
  return {
    supabase: {
      from: () => ({
      ...
      channel: () => ({
        on: (_event: any // eslint-disable-line @typescript-eslint/no-explicit-any, _filter: any // eslint-disable-line @typescript-eslint/no-explicit-any, cb: () => void) => {
          subscribers.push(cb);
